...
| `KEYCLOAK_CLIENT_ID` | `agent-dashboard`            | Client ID           |
...
| `MINIO_BUCKET`     | `insureportal-agent-dashboard`  | Default bucket |
...
| `KAFKA_CLIENT_ID` | `agent-dashboard-server`    | Client ID      |
| `KAFKA_GROUP_ID`  | `agent-dashboard-consumers` | Consumer group |
...
| `VAULT_SECRET_PATH` | `secret/data/agent-dashboard` | Secret mount path    |
...
| `TEMPORAL_NAMESPACE`  | `agent-dashboard`       | Namespace               |
| `TEMPORAL_TASK_QUEUE` | `agent-dashboard-tasks` | Task queue name         |
