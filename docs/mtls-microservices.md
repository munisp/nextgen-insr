...
└── InsurePortal Intermediate CA  (online, rotated every 90 days)
    ├── agent-dashboard.svc.insureportal.internal
...
```yaml
# k8s/certs/agent-dashboard-cert.yaml
apiVersion: cert-manager.io/v1
kind: Certificate
metadata:
  name: agent-dashboard-mtls
  namespace: insureportal
spec:
  secretName: agent-dashboard-mtls-tls
  duration: 720h # 30 days
  renewBefore: 168h # Renew 7 days before expiry
  subject:
    organizations: ["InsurePortal"]
  commonName: agent-dashboard.svc.insureportal.internal
  dnsNames:
    - agent-dashboard.svc.insureportal.internal
    - agent-dashboard.insureportal.svc.cluster.local
  issuerRef:
...
```yaml
# dapr/components/agent-dashboard-subscription.yaml
apiVersion: dapr.io/v1alpha1
kind: Subscription
metadata:
  name: agent-dashboard-events
spec:
...
openssl s_client \
  -connect agent-dashboard.svc.insureportal.internal:8443 \
