...
func initDB() {
	...
	log.Printf("Connected to PostgreSQL for multi_currency_support")

	// Create table if not exists
	_, err = db.Exec(`CREATE TABLE IF NOT EXISTS multi_currency_support (
...
func main() {
	...
	r.Get("/health", func(w http.ResponseWriter, r *http.Request) {
		json.NewEncoder(w).Encode(map[string]string{"status": "healthy", "database": fmt.Sprintf("%v", db != nil), "service": "multi-currency-support", "version": "1.0.0"})
	})
