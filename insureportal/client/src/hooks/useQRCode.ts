 ...
 *  5. Parse InsurePortal QR payload format: INSUREPORTAL:{ref}:{amount}:{agentId}
 *
 * Offline strategy:
...
export interface OfflineQRRecord {
  ...
  agentId: string;
  label: string;
  payload: string; // INSUREPORTAL:{ref}:{amount}:{agentId}
  createdAt: string;
  synced: boolean;
}
...
export function parseQRPayload(raw: string): ParsedQRPayload {
  // InsurePortal format: INSUREPORTAL:{ref}:{amount}:{agentId}
...
export function buildInsurePortalQRPayload(
  ...
  return `INSUREPORTAL:${ref}:${amount}:${agentId}`;
}
