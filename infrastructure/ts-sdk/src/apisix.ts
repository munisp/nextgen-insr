...
export class APISixClient {
  ...
  async ping(): Promise<void> {
    ...
    if (resp.status >= 500) throw new Error(`APISix unhealthy: ${resp.status}`);
  }

  async createRoute(routeId: string, uri: string, name: string, methods: string[], upstreamUrl: string, plugins?: Record<string, unknown>): Promise<void> {
    const body = {
      uri, name, methods,
  ...
  private defaultPlugins(): Record<string, unknown> {
    return {
  ...
  async registerPlatformRoutes(): Promise<void> {
