...
func TestIntegration_DBConnection(t *testing.T) {
	...
	// Verify table exists
	var exists bool
	err := testDB.QueryRow("SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_name=$1)", "multi_tenant_platform").Scan(&exists)
	if err != nil {
		t.Fatalf("Failed to check table existence: %v", err)
	}
	if !exists {
		t.Fatalf("Table multi_tenant_platform does not exist")
	}
}
...
func TestIntegration_APIEndpoint(t *testing.T) {
	...
	body := `{"tenantName":"IntTestCorp","domain":"inttest.insureportal.ng","plan":"enterprise"}`
	...
	_ = fmt.Sprintf("Integration test completed for multi-tenant-platform")
}
