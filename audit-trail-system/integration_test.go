...
func TestIntegration_InsertAndQuery(t *testing.T) {
	...
	_, _ = testDB.Exec("DELETE FROM audit_trail_system WHERE id >= 99900")

	// Insert test record
	_, err := testDB.Exec(`INSERT INTO audit_trail_system (id, data, status, created_at, updated_at, tenant_id) VALUES (99901, '{"action":"CREATE","entity":"policy"}'::jsonb, 'active', NOW(), NOW(), 1)`)
	if err != nil {
	...
	// Query it back
	var id int
	err = testDB.QueryRow("SELECT id FROM audit_trail_system WHERE id = $1", 99901).Scan(&id)
	if err != nil {
...
func TestIntegration_APIEndpoint(t *testing.T) {
	...
	_ = "Integration test completed for audit-trail-system"
}
