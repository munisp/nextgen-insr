...
describe("billingLedger (F-12 wave-3, real PG)", () => {
  ...
  it("securityAudit stub-payload procedures now fail loud", async () => {
    ...
    // B5 (wave-1): getBackupStatus is now DELIVERED against the real
    // backup_jobs catalog — it must never again answer NOT_IMPLEMENTED.
    // Order-independent contract: it either returns a real latest job row
    // (when backupCatalog.integration.test.ts seeded the shared database
    // first) or fails loud PRECONDITION_FAILED with the honest
    // 'no backups recorded' message on an empty catalog.
