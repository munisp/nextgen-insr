...
| `server/sprint82.test.ts` | 8 billing-provisioning activity exports + BillingProvisioningWorkflow implemented 2026-08-17 against the REAL billing tables (mirroring executeBillingProvisioning; rollback = real compensating mutations audited in billing_provisioning_history) — verified 24/25; file stays excluded ONLY for CAT-A `services/go/billing-provisioning-workflow/main.go` (zero-commit Go sidecar, undelivered scope) |
