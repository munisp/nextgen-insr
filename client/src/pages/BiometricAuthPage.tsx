...
export default function BiometricAuthPage() {
  // F-12 (wave-4b): biometric authentication is not delivered — the procs are fail-loud
  // NOT_IMPLEMENTED. Honest unavailable state; the query stays wired.
  const { isError } = trpc.biometricAuth.list.useQuery(undefined, {
    retry: false,
  });
    ...
    <div className="p-6 space-y-6">
      ...
      <Card>
        <CardContent className="py-10 text-center text-muted-foreground">
          — biometric authentication is not delivered on this platform
          {isError && (
