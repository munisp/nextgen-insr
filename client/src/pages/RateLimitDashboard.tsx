...
export default function RateLimitDashboard() {
  ...
  const cards = [
    ...
    { title: "Throttled (1h)", value: d.throttledLastHour ?? "—", icon: AlertTriangle, trend: "flat" as const, trendValue: "blocked", status: (Number(d.throttledLastHour ?? 0) > 100 ? "warning" : "good") as const, href: "/rate-limit-dashboard", accent: "var(--risk-medium)" },
    { title: "Throttle Rate (%)", value: d.throttleRate ? d.throttleRate.toFixed(2)+"%" : "—", icon: Activity, trend: "down" as const, trendValue: "↓ 0.1%", status: (Number(d.throttleRate ?? 0) > 5 ? "warning" : "good") as const, href: "/rate-limit-dashboard", accent: "var(--risk-low)" },
