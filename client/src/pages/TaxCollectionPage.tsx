...
export default function TaxCollectionPage() {
  // F-12 (wave-4b): no tax collection store is delivered — the procs are fail-loud
  // NOT_IMPLEMENTED. Honest unavailable state; the query stays wired.
  const { isError } = trpc.taxCollection.history.useQuery(undefined, {
    retry: false,
  });
    ...
    <div className="p-6 space-y-6">
      ...
      <Card>
        <CardContent className="py-10 text-center text-muted-foreground">
          — tax collection is not delivered on this platform
          {isError && (
