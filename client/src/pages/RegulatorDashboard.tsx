...
export default function RegulatorDashboard() {
  ...
  const cards = [
    ...
    { title: "Claims Ratio (%)", value: kpi.claimsRatio ? kpi.claimsRatio.toFixed(1)+"%" : "—", icon: Activity, trend: "down" as const, trendValue: "↓ 2%", status: (Number(kpi.claimsRatio??0)>80?"critical":"good") as const, href: "/claims", accent: "var(--risk-low)" },
    { title: "Capital Adequacy (%)", value: cap.ratio ? cap.ratio.toFixed(1)+"%" : "—", icon: TrendingUp, trend: "up" as const, trendValue: "NAICOM min 15%", status: (Number(cap.ratio??0)>=15?"good":"critical") as const, href: "/regulatory-compliance-checks", accent: "var(--risk-low)" },
    { title: "Compliance Score", value: comp.overallScore ? comp.overallScore+"%" : "—", icon: CheckCircle, trend: "up" as const, trendValue: "↑ 1.5%", status: (Number(comp.overallScore??0)>=90?"good":"warning") as const, href: "/compliance-dashboard", accent: "var(--risk-low)" },
