...
export default function RealtimePnlDashboard() {
  ...
  const cards = [
    ...
    { title: "Net P&L (₦M)", value: d.netPnl ? (d.netPnl/1e6).toFixed(2) : "—", icon: Activity, trend: (Number(d.netPnl ?? 0) >= 0 ? "up" : "down") as const, trendValue: Number(d.netPnl ?? 0) >= 0 ? "profit" : "loss", status: (Number(d.netPnl ?? 0) >= 0 ? "good" : "critical") as const, href: "/financial-reporting-suite", accent: Number(d.netPnl ?? 0) >= 0 ? "var(--risk-low)" : "var(--risk-critical)" },
    { title: "Combined Ratio (%)", value: d.combinedRatio ? d.combinedRatio.toFixed(1)+"%" : "—", icon: BarChart2, trend: "down" as const, trendValue: "↓ 2%", status: (Number(d.combinedRatio ?? 100) < 100 ? "good" : "critical") as const, href: "/financial-reporting-suite", accent: "var(--insurance-secondary)" },
