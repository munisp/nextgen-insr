...
export default function SavingsProductsPage() {
  // F-12 (wave-4b): no savings products store is delivered — the procs are fail-loud
  // NOT_IMPLEMENTED. Honest unavailable state; the query stays wired.
  const { isError } = trpc.savingsProducts.list.useQuery(undefined, {
    retry: false,
  });
    ...
    <div className="p-6 space-y-6">
      ...
      <Card>
        <CardContent className="py-10 text-center text-muted-foreground">
          — savings products is not delivered on this platform
          {isError && (
