...
func initDB() {
	...
	log.Printf("Connected to PostgreSQL for insurance_radar")

	// Create table if not exists
	_, err = db.Exec(`CREATE TABLE IF NOT EXISTS insurance_radar (
...
func main() {
	...
	r.Get("/health", func(w http.ResponseWriter, r *http.Request) {
		json.NewEncoder(w).Encode(map[string]string{"status": "healthy", "database": db != nil, "service": "insurance-radar", "version": "1.0.0"})
	})
