...
func (s *Server) getFloodRiskZones(c *gin.Context) {
	...
	for rows.Next() {
		...
		_ = json.Unmarshal([]byte(boundaryGeoJSON), &boundary)
...
func (s *Server) createClaimLocation(c *gin.Context) {
	...
	_ = s.db.QueryRow(c.Request.Context(), distanceQuery, req.PolicyID, req.Longitude, req.Latitude).Scan(&distanceFromPolicy)
...
func (s *Server) getAgentTerritories(c *gin.Context) {
	...
	for rows.Next() {
		...
		if territoryGeoJSON != nil {
			_ = json.Unmarshal([]byte(*territoryGeoJSON), &territory)
		}
