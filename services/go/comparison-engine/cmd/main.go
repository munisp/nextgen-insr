...
func main() {
	...
	mux.HandleFunc("/api/v1/compare", func(w http.ResponseWriter, r *http.Request) {
		...
		if len(filtered) > 0 {
			...
			for _, p := range filtered {
				if p.Premium < products[0].Premium { result.Cheapest = p.ID }
				if p.Coverage > products[0].Coverage { result.BestCoverage = p.ID }
				if p.Rating > products[0].Rating { result.BestValue = p.ID }
			}
		}
		w.Header().Set("Content-Type", "application/json")
	...
	mux.HandleFunc("/api/v1/recommend", func(w http.ResponseWriter, r *http.Request) {
		...
		json.NewDecoder(r.Body).Decode(&needs)

		budget := int64(float64(needs.Income) * needs.BudgetPct / 100 * 12)
		var recs []Recommendation
		for _, p := range products {
			if p.Premium > budget { continue }
			score := 0.5
			reasons := []string{}
			for _, concern := range needs.Concerns {
				if p.Category == concern { score += 0.3; reasons = append(reasons, "Matches your "+concern+" concern") }
			}
			if p.ClaimRatio > 0.85 { score += 0.1; reasons = append(reasons, "High claim settlement ratio") }
			if needs.Dependents > 0 && p.Category == "life" { score += 0.2; reasons = append(reasons, "Protects your family") }
