...
type GammaConfig struct {
	...
	ReferralRewardPts  int
	QualificationDays  int
	MaxPointsPerDay    int
	ChallengeMinDays   int
	LeaderboardRegion  bool
}

func NewConfig() *Config {
	return &Config{
		Server: ServerConfig{
			Port: envOr("PORT", "8125"), Host: envOr("SERVER_HOST", "0.0.0.0"),
		...
		Postgres: PostgresConfig{
			...
			DBName: envOr("DB_NAME", "gamification"), MaxIdleConns: intOr("DB_MAX_IDLE_CONNS", 10),
		...
		Gamma: GammaConfig{
			PointsPerNaira: floatOr("POINTS_PER_NAIRA", 2),
			DailyReferralLimit: intOr("DAILY_REFERRAL_LIMIT", 5),
			ReferralRewardPts: intOr("REFERRAL_REWARD_POINTS", 200),
			QualificationDays: intOr("QUALIFICATION_DAYS", 30),
			MaxPointsPerDay: intOr("MAX_POINTS_PER_DAY", 1000),
			ChallengeMinDays: intOr("CHALLENGE_MIN_DAYS", 7),
			LeaderboardRegion: boolOr("LEADERBOARD_REGION", true),
		},
	}
}
...
func parseCSV(s string) []string {
	...
	start := 0
	for i := 0; i < len(s); i++ {
		if s[i] == ',' { r = append(r, s[start:i]); start = i + 1 }
	}
	if start < len(s) { r = append(r, s[start:]) }
	return r
}
