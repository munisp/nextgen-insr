...
func TestIntegration_DBConnection(t *testing.T) {
	...
	// Verify table exists
	var exists bool
	err := testDB.QueryRow("SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_name=$1)", "gamification_service").Scan(&exists)
	if err != nil {
		t.Fatalf("Failed to check table existence: %v", err)
	}
	if !exists {
		t.Fatalf("Table gamification_service does not exist")
	}
}
...
func TestIntegration_InsertAndQuery(t *testing.T) {
	...
	testDB.Exec("DELETE FROM gamification_service WHERE id LIKE 'int-test-%'")

	// Insert test record
	_, err := testDB.Exec("INSERT INTO gamification_service (id, user_id, points, level, badge, created_at) VALUES ('int-test-game-1', 'USR-GAM-001', 100, 1, 'newcomer', NOW())")
	if err != nil {
	...
	// Query it back
	var id string
	err = testDB.QueryRow("SELECT id FROM gamification_service WHERE id = $1", "int-test-gamification-1").Scan(&id)
	if err != nil {
...
func TestIntegration_APIEndpoint(t *testing.T) {
	...
	mux.HandleFunc("/api/v1/award", func(rw http.ResponseWriter, r *http.Request) {
		rw.Header().Set("Content-Type", "application/json")
	...
	_ = fmt.Sprintf("Integration test completed for gamification-service")
}
