...
export const merchantPaymentsRouter = router({
  pay: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      const [{ dailyTotal }] = await db.select({ dailyTotal: sql<string>`COALESCE(SUM(CAST(amount AS NUMERIC)), 0)` })
        .from(transactions).where(and(eq(transactions.agentId, input.agentId), eq(transactions.type, "Merchant Payment"), gte(transactions.createdAt, today), eq(transactions.status, "success")));
      ...
      try {
        ...
        const [tx] = await db.insert(transactions).values({
          reference: input.reference, agentId: input.agentId, type: "Merchant Payment",
          amount: String(input.amountNGN), fee: String(mdrFee), commission: "0",
          channel: input.paymentMethod, status: "success", fraudScore: "0.00",
          tbSyncStatus: tbResult ? "synced" : "pending",
          metadata: { merchantId: input.merchantId, mdrFee, merchantAmount, description: input.description ?? null, tbTransferId: tbResult?.id ?? null },
        }).returning();
  ...
  list: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const results = await db.select().from(transactions).where(eq(transactions.type, "Merchant Payment")).orderBy(desc(transactions.createdAt)).limit(input.limit).offset(input.offset);
      const [{ total }] = await db.select({ total: count() }).from(transactions).where(eq(transactions.type, "Merchant Payment"));
  ...
  getSummary: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      }).from(transactions).where(and(eq(transactions.type, "Merchant Payment"), gte(transactions.createdAt, since), eq(transactions.status, "success")));
