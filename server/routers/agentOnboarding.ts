      ...
      try {
        ...
        if (agent.email) {
          const { subject, html, text } = buildAlertEmail({
            ...
            message: `Congratulations ${agent.name}! Your InsurePortal POS agent account (${input.agentCode}) has been fully activated. You can now process transactions on your terminal.`,
            severity: "low",
          });
