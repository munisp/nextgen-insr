...
export function buildWelcomeEmail(data: {
  ...
  const html = `<!DOCTYPE html><html><body><div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto"><div style="background:#1a1a2e;color:white;padding:20px;text-align:center"><h1 style="margin:0">InsurePortal</h1><p>Welcome to the Platform</p></div><div style="padding:20px"><p>Dear ${data.agentName},</p><p>Welcome to InsurePortal! Your agent code: <strong>${data.agentCode}</strong></p></div></div></body></html>`;
  return {
    to: "",
    subject: `Welcome to InsurePortal — Agent ${data.agentCode}`,
    html,
...
export function buildPasswordResetEmail(data: {
  ...
  return {
    to: "",
    subject: "PIN Reset OTP — InsurePortal",
    html,
...
export function buildDigestEmail(data: {
  ...
  const itemsHtml = data.items
    ? data.items
        ...
        .join("")
    : "";
  const html = `<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto"><div style="background:#1a1a2e;color:white;padding:20px;text-align:center"><h1 style="margin:0">InsurePortal</h1><p>${data.period} Digest</p></div><div style="padding:20px"><p>Hello ${name},</p><p>Here's your ${data.period} summary:</p>${data.txCount !== undefined ? `<p>Transactions: ${data.txCount}</p>` : ""}${itemsHtml ? `<table style="width:100%">${itemsHtml}</table>` : ""}</div></div>`;
  return {
    to: "",
    subject: `${data.period} Digest — InsurePortal`,
    html,
