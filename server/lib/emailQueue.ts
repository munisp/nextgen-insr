 ...
 *   SMTP_FROM        - From address (e.g., "InsurePortal <noreply@insureportal.io>")
...
const DEFAULT_FROM = process.env.SMTP_FROM ?? "InsurePortal <noreply@insureportal.io>";
...
export function buildTransactionReceiptEmail(opts: {
  ...
  const html = `
    <div style="font-family:sans-serif;max-width:480px;margin:0 auto;padding:24px;border:1px solid #e5e7eb;border-radius:8px;">
      <h2 style="color:#1d4ed8;margin-bottom:4px;">InsurePortal</h2>
      ...
      <hr style="border:none;border-top:1px solid #e5e7eb;margin:16px 0;" />
      <p style="color:#6b7280;font-size:12px;margin:0;">This is an automated receipt from InsurePortal. Do not reply to this email.</p>
    </div>
  `;

  const text = `InsurePortal — Transaction Receipt\n\nRef: ${opts.ref}\nType: ${opts.type}\nAmount: ${amountStr}\nFee: ${feeStr}\nCommission: ${commStr}\nAgent: ${opts.agentName} (${opts.agentCode})\nDate: ${opts.timestamp.toLocaleString("en-NG")}`;
